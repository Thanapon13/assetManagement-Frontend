              ...
              <div className="flex justify-center">
                {item.status === "waiting" || item.status === "saveDraft" ? (
                  <div className="flex gap-1">
                    <Link
                      to={item.status == "saveDraft" ? `/borrowRecord/${item._id}` : `/borrowEdit/${item._id}`}
                    ...
                    <ModalCancel item={item} fetchLists={props.fetchLists} />
                  </div>
                ) : (
                  <Link
...
const ModalCancel = ({ item,fetchLists }) => {
  ...
  return (
    <>
      <button
        className="border-[1px] border-red-600 focus:border-transparent shadow-sm font-medium  text-red-600 hover:bg-red-100 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-red-600  h-[31px] w-[31px] flex justify-center items-center rounded-md"
        type="button"
          ...
          <div className="flex justify-center items-center overflow-y-auto fixed top-0 pt-[15vh] md:pt-0 bottom-0 left-0 z-40 md:inset-0 md:w-screen">
            <div className="w-10/12 md:w-7/12 max-w-[1040px] border border-white shadow-md rounded-xl ">
              <div className="rounded-lg shadow-lg flex flex-col w-full bg-white">
                ...
                <div className="grid grid-cols-2 md:grid-cols-6 p-2 text-normal">
                ...
                <div className="grid grid-cols-2 md:grid-cols-6 p-2 pb-4">
                  <div className="text-text-gray flex items-center">
                  ...
                  <Link
                    ...
                    onClick={() => {
                      item.status == "saveDraft"
                        ...
                        : remark ? confirmDelete(item._id, remark) : setError(true)
                    }}
                  >
                    ยืนยัน
                  </Link>
