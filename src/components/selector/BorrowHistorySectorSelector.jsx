...
const BorrowHistorySectorSelector = ({
  placeholder,
  fetchDataDropdown,
  state,
  setState,
      ...
      >
        ...
        {data?.map((data,idx) => (
          <li
            ...
            ${
              data?.sector.toLowerCase() === state[id]?.toLowerCase() &&
              "bg-sky-600 text-white"
            }
            ${
              data?.sector.toLowerCase().startsWith(inputValue)
                ? "block"
                : "hidden"
            }`}
            onClick={() => {
              if (data?.sector.toLowerCase() !== state[id]?.toLowerCase()) {
                handleChange(data.sector);
                // setState(data?.[id]);
                // console.log(state);
...
export default BorrowHistorySectorSelector;
